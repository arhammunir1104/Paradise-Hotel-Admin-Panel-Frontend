...
function ConfirmReservationCard({data}) {
    ...
    let {closeReservation, cancelReservation} = useAuth();
    ...
    async function closereservation(room_id, reservation_id){
        ...
        try{    
            ...
            let data = await closeReservation(room_id, reservation_id);
            if(data.status === true || data){
                // alert(`Reservation with ID ${reservation_id} has been Closed.`);
                // window.location.reload();
                toast.success(`Reservation with ID ${reservation_id} has been Closed.`, {
                    position: "top-right",
                    autoClose: 2000,
                ...
                // alert(`Closing Reservation with ID ${reservation_id} has been Failed.`);
                toast.error(`Closing Reservation with ID ${reservation_id} has been Failed.`, {
                    position: "top-right",
                    autoClose: 3000,
                ...
                <div className='grid md:grid-cols-7 grid-cols-1 shadow-lg pl-[1%]'>
                    ...
                    <div className='col-span-2 flex justify-center flex-col'>
                        <div className='flex  md:mt-[20%] mt-[5%] items-center p-[10px] justify-center gap-2'>
                            ...
                            <Button variant='outlined' style={{borderColor: "red", color: "red", width: "100%"}} onClick={handleOpen} >Cancel Reservation</Button>
                            </div>
                            <div  className='flex-[1]'>
                            <Button variant="contained" style={{backgroundColor : "green",  width: "100%"}} onClick={handleOpen2} >Close Reservation</Button>
                            </div>
                        </div>
            ...
            >
                ...
                <div>
                    ...
                    <div className='my-[20px] flex justify-end'>
                        ...
                        <Button variant="contained" style={{backgroundColor : "red"}}  onClick={()=>{cancelreservation(data.room_id, data._id, "confirmed" )}} >Yes</Button>
                    </div>
                </Box>
            </Modal>
        ...
        <Box sx={style}>
          <div>
            ...
            <div className='my-[20px] flex justify-end'>
                ...
                <Button variant="contained" style={{backgroundColor : "green"}} onClick={()=>{closereservation(data.room_id, data._id )}} >Yes</Button>
            </div>
        </Box>
      </Modal>
